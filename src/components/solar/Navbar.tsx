...
const SOCIAL_LINKS = [
  { href: 'https://www.facebook.com/SolarIrelandGroup', label: 'Facebook', color: 'hover:text-[#1877F2] hover:border-[#1877F2]/30' },
  ...
  { href: 'https://www.threads.com/@solarireland', label: 'Threads', color: 'hover:text-white hover:border-white/30' },
] as const;
            ...
            <div className="flex items-center justify-center gap-3 mt-4">
              {SOCIAL_LINKS.map((s) => (
                ...
                >
                  ...
                  {s.label === 'Threads' && (
                    <svg className="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="currentColor"><path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.291 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.589 12c.027 3.086.718 5.496 2.057 7.168 1.43 1.783 3.631 2.698 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.75-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.964-.065-1.19.408-2.285 1.33-3.082.88-.76 2.119-1.207 3.583-1.291a13.856 13.856 0 013.02.142c-.126-.742-.375-1.332-.75-1.757-.513-.586-1.308-.883-2.359-.89h-.029c-.844 0-1.992.232-2.721 1.32L9.696 8.19c.98-1.454 2.568-2.256 4.478-2.256h.044c3.194.02 5.097 1.975 5.287 5.388.108.046.216.094.321.142 1.49.7 2.58 1.761 3.152 3.07.797 1.82.871 4.79-1.548 7.158-1.85 1.81-4.094 2.628-7.256 2.65z"/></svg>
                  )}
                </a>
              ))}
            </div>
      ...
      >
        <div className="max-w-6xl xl:max-w-[74rem] 2xl:max-w-[78rem] mx-auto px-4 sm:px-6 lg:px-8 h-[72px] xl:h-20 flex items-center justify-between gap-4 lg:gap-6 xl:gap-8">
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <button
              onClick={() => navigateTo('/#calculator')}
              className="hidden xl:inline-flex bg-amber-400 hover:bg-amber-300 text-black font-bold text-xs px-5 py-2 xl:py-2.5 rounded-full uppercase tracking-wider active:scale-95 transition-all shadow-lg shadow-yellow-400/10 hover:shadow-yellow-400/20 whitespace-nowrap"
            >
              Analyse Bill
            </button>
            <button
              onClick={() => navigateTo('/book-survey')}
              className="bg-green-400 hover:bg-green-300 text-black font-bold text-xs px-5 py-2 xl:py-2.5 rounded-full uppercase tracking-wider active:scale-95 transition-all shadow-lg shadow-green-400/10 hover:shadow-green-400/20 whitespace-nowrap"
            >
              Book Free Survey
            </button>
          </div>
