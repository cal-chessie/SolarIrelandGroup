...
/* ═══════════════════════════════════════════
   MAIN COMPONENT — ZERO Framer Motion
   ═══════════════════════════════════════════ */
export default function WhatsAppChat() {
  ...
  // ─── Determine if chat should render (for CSS transition) ───
  const showChat = isOpen;

  return (
    <>
      {/* ═══════════════════════════════════════
          NOTIFICATION TOAST — CSS transition
          ═══════════════════════════════════════ */}
      <div
        onClick={handleOpen}
        className={`fixed bottom-24 right-6 z-50 cursor-pointer max-w-xs transition-all duration-300 ease-out ${
          notification && !isOpen
      ...
      {/* ═══════════════════════════════════════
          CHAT PANEL — CSS transition, no Framer Motion
          ═══════════════════════════════════════ */}
      <div
        className={`fixed z-50 flex flex-col overflow-hidden shadow-2xl shadow-black/40 border border-white/[0.08] transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
          showChat
          ...
          <>
            ...
            >
              ...
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} chat-msg-appear`}
                >
                  {msg.role === 'assistant' && (
              ...
              {/* Typing indicator */}
              {isLoading && (
                <div className="flex justify-start chat-msg-appear">
                  <div className="w-7 h-7 rounded-full bg-white/[0.04] flex items-center justify-center mr-2 mt-1 shrink-0 overflow-hidden">
                    <img src="/bumblebee-sm.png" alt="" className="w-5 h-5" />
                  </div>
