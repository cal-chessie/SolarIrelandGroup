      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 mb-6">
          {service.features.slice(0, 4).map((f) => (
            <div key={f.label} className="flex items-baseline gap-x-2 min-w-0">
              <span className="text-[10px] text-gray-600 uppercase tracking-wider whitespace-nowrap shrink-0">{f.label}</span>
              <span className="text-sm text-gray-300 font-medium leading-snug truncate">{f.value}</span>
            </div>
          ))}
        </div>
